...
@media (prefers-color-scheme: dark) {
  ...
  .post-time {
    color: var(--tg-text-secondary-dark);
  }
  ...
  /* Стили для боковых панелей (если они есть на странице) */
  .sidebar-card {
    background: var(--tg-card-bg-dark);
    border: 1px solid var(--tg-border-dark);
  }

  .user-bio, .sidebar-title, .sidebar-menu a {
    color: var(--tg-text-primary-dark);
  }
  ...
  .post-input-field, .comment-input {
    background: var(--tg-input-bg-dark);
    color: var(--tg-text-primary-dark);
  }

  .post-input-field:focus, .comment-input:focus {
  ...
  .post-action:hover, .post-action-btn:hover {
    background: var(--tg-hover-dark);
  }
  ...
  .comments-section, .view-all-link {
    border-color: var(--tg-border-dark);
  }
